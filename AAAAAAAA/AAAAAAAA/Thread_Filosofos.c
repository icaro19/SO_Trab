/*
Filosofos glutoes com threads
gcc Thread_Filosofos.c -o threadfilo.c -Wall -lpthread
entrada: numero de filosofos, tempo de pensamento e tempo comendo
...
//		printf("id_filosofo = %d\n", id_filosofo[k]);
	...
	for (int j = 0; j < num_filosofos; ++j)//gera thread para cada filosofo
	{
...
//		printf("\n-----filosofos - thread %d-----\n", pthread_self());
		int i = * (int *) fil;
		ESQ = (id_filosofo[i]+num_filosofos-1)%num_filosofos;//definicao de esquerda e direita
		DIR = (id_filosofo[i]+1)%num_filosofos;
...
//	printf("ESQ = %d\tDir = %d\n", ESQ, DIR);
	teste(ESQ);//verifica se os filosofos ao lado podem se alimentar ou nao
	teste(DIR);
}
...
//	printf("\n-----teste-----\n");
	if (estado[indice] == FAMINTO && estado[ESQ] != COMENDO && estado[DIR] != COMENDO)//verifica se os garfos ao lado estão livres
	{
